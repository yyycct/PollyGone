...
namespace DigitalRuby.WeatherMaker
{
    ...
    public class WeatherMakerShadowMapScript : MonoBehaviour
    {
        ...
        private void AddScreenSpaceShadowsCommandBuffer(Camera camera)
        {
                ...
                WeatherMakerScript.Instance.PerformanceProfile.VolumetricCloudShadowSampleCount > 0)
            {
                ...
                if (_commandBufferScreenSpaceShadows1 == null)
                {
                    ...
                    commandBufferScreenSpaceShadowsDictionary1[camera] = _commandBufferScreenSpaceShadows1 = new CommandBuffer { name = "WeatherMakerShadowMapScreensSpaceShadowScriptBlur_" + gameObject.name + "_" + camera.CachedName() };
                }
                ...
                {
                    // copy the screen space shadow texture for re-use later
                    commandBufferScreenSpaceShadowsDictionary2[camera] = _commandBufferScreenSpaceShadows2 = new CommandBuffer { name = "WeatherMakerShadowMapScreensSpaceShadowScriptBlit_" + gameObject.name + "_" + camera.CachedName() };
                }
                ...
                {
                    camera.RemoveCommandBuffer(CameraEvent.BeforeForwardOpaque, commandBufferScreenSpaceShadowsList1[commandBufferScreenSpaceShadowsList1.Count - 1]);
                }
                ...
                {
                    camera.RemoveCommandBuffer(CameraEvent.BeforeForwardOpaque, commandBufferScreenSpaceShadowsList2[commandBufferScreenSpaceShadowsList2.Count - 1]);
                }
        ...
        private void CameraPostRender(Camera camera)
        {
            ...
            if (commandBufferScreenSpaceShadowsList1.Count != 0)
            {
                camera.AddCommandBuffer(CameraEvent.BeforeForwardOpaque, commandBufferScreenSpaceShadowsList1[commandBufferScreenSpaceShadowsList1.Count - 1]);
            }
            if (commandBufferScreenSpaceShadowsList2.Count != 0)
            {
                camera.AddCommandBuffer(CameraEvent.BeforeForwardOpaque, commandBufferScreenSpaceShadowsList2[commandBufferScreenSpaceShadowsList2.Count - 1]);
            }
