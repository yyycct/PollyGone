...
)
{
	...
	fixed dithering = state.dithering;
	...
	float3 marchDir;
	marchPos += (rayDir * state.dithering * _CloudRayDitherVolumetric * 256.0);
	...
	while (i++ < sampleCount && cloudColor.a < VOLUMETRIC_CLOUD_MAX_ALPHA && heightFrac >= -0.01 && heightFrac <= 1.01)
	{
		...
		if (CloudVolumetricGetCoverage(weatherData) > _CloudCoverVolumetricMinimumForCloud)
		{
			...
			if (cloudSample > VOLUMETRIC_CLOUD_MIN_NOISE_VALUE)
			{
				...
				if (cloudSample > VOLUMETRIC_CLOUD_MIN_NOISE_VALUE)
				{
					...
					/*
					float depth01 = distance(WEATHER_MAKER_CAMERA_POS, marchPos) * atmosphere01;
		...
		marchPos += (marchDir * marchMultiplier * (1.0 + ((dithering *= 0.98) * _CloudRayDitherVolumetric * 0.5)));
	}
...
CloudColorResult ComputeCloudColorVolumetric(float3 rayDir, float4 uv, float depth, float depth01, inout CloudState state)
{
	...
	uint iterations = SetupCloudRaymarch(WEATHER_MAKER_CAMERA_POS, cloudRayDir, depth, depth,
