...
uniform float _CloudTopFade;
uniform float _CloudRoundnessVolumetric;
uniform float _CloudRoundnessVolumetricInv;
uniform float _CloudCoverageHeightStart;
uniform float _CloudCoverageHeightMultiplier;
...
static const float3 volumetricCloudPlanePos = float3(WEATHER_MAKER_CAMERA_POS.x, _CloudHeightVolumetric * 0.5, WEATHER_MAKER_CAMERA_POS.z);
...
static const float volumetricIsAboveClouds = WEATHER_MAKER_CAMERA_POS.y > _CloudEndVolumetric;
static const float volumetricIsAboveMiddleClouds = WEATHER_MAKER_CAMERA_POS.y > (_CloudStartVolumetric + _CloudEndVolumetric) * 0.5;
static const float volumetricIsBelowClouds = WEATHER_MAKER_CAMERA_POS.y < _CloudStartVolumetric;
static const float volumetricAboveClouds = saturate(1.0 - ((WEATHER_MAKER_CAMERA_POS.y - _CloudEndVolumetric) * _CloudStartInverseVolumetric));
static const float volumetricAboveCloudsSquared = volumetricAboveClouds * volumetricAboveClouds;
static const float volumetricBelowClouds = saturate(WEATHER_MAKER_CAMERA_POS.y * _CloudStartInverseVolumetric);
...
static const float3 volumetricWindDir2 = volumetricWindDir1 * 25.0;
